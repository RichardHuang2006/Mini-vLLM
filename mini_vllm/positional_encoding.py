...
The interface carries the design decision: :meth:`RoPE.__call__` takes the absolute
position of every token as a tensor and never assumes ``arange(L)``.

That is what makes the serving layer possible. A decode step for a sequence at position
500 and a prefill chunk covering positions 0-511 share a single forward pass, so position
is a property of each token rather than of the batch. An implicit ``arange`` produces
correct prefill and a subtly wrong continuation.
"""
...
def apply_rope(
    ...
    kernel has the same signature, which lets `mini_vllm.kernels.ops.rope` dispatch
    between the two without either side knowing about the other.
    """
