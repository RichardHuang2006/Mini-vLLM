...
* :func:`scaled_dot_product_attention_simple`, the textbook form with `H_q == H_k`.
* :func:`scaled_dot_product_attention_grouped`, the GQA form Qwen3 uses, with fewer
  key/value heads than query heads.

Both materialize the whole `L x S` score matrix, which is what the FlashAttention-style
prefill and decode kernels avoid. They exist as the oracles those kernels are diffed
against, so they are written for clarity rather than speed.
"""
...
def scaled_dot_product_attention_simple(
    ...
    ``scale`` defaults to ``1/sqrt(D)``. The mask is additive rather than boolean
    because additive masks compose: adding ``-inf`` before the softmax drives a
    position to exactly zero afterwards, and several masks can be summed.
    """
    head_dim = q.shape[-1]
    if scale is None:
...
class SimpleMultiHeadAttention:
    ...
    ``H·D`` need not equal ``E``: in Qwen3-0.6B the attention projection is twice the
    hidden size, so the head dimension is derived from the weight rather than from
    ``E / H``.

    Superseded by the Qwen3 attention block, which adds QK-norm, RoPE, GQA and a cache.
    Retained as the smallest form checkable directly against
